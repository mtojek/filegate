...
func main() {
	...
	// pasted below:
	fmt.Println("Enter receiver's offer:")
	reader := bufio.NewReader(os.Stdin)
	serializedOffer, _ := reader.ReadString('\n')

	var offer webrtc.RTCSessionDescription
	json.NewDecoder(bytes.NewBuffer([]byte(serializedOffer))).Decode(&offer)

	peerConnection.SetRemoteDescription(offer)

	fmt.Println("Sender's answer:")
